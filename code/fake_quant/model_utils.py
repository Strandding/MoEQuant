...
def _is_qwen_moe_model(model):
    """Check if a model is a Qwen MoE model (Qwen1.5, Qwen2, Qwen3, or Qwen3-Next)."""
    model_class_name = model.__class__.__name__
    # Check for Qwen2MoeForCausalLM, Qwen3MoeForCausalLM, Qwen3NextForCausalLM, or custom Qwen MoE models
    # Note: Qwen3Next also has MoE architecture even though 'Moe' is not in the class name
    if any(pattern in model_class_name for pattern in ['QwenMoe', 'Qwen2Moe', 'Qwen3Moe', 'Qwen3Next']):
        return True
    # Fallback to checking if it's an instance of the standard Qwen2MoeForCausalLM
...
def _is_qwen_moe_type(model_type):
    ...
    if model_type is not None:
        ...
        # Check for Qwen MoE variants including Qwen3Next
...
def _is_deepseek_model(model):
    ...
    model_class_name = model.__class__.__name__
    # Check for DeepseekForCausalLM (V1) or DeepseekV2ForCausalLM (V2)
    if 'Deepseek' in model_class_name:
        return True
    # Fallback to checking if it's an instance of the standard DeepseekForCausalLM
...
def model_type_extractor(model):
    if isinstance(model, LLAMA_MODEL):
        ...
        # Return the dynamically loaded class if available, otherwise the default
...
def get_qwen(model_name, hf_token, args):
    ...
    # Dynamically set the model and layer classes based on the loaded model
    _QWEN_MODEL_CLASS = model.__class__
    # Find the decoder layer class by inspecting the first layer
    ...
    # For Qwen3 and other models where rotary_emb is at model level, pass it to args
