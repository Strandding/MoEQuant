...
class QuantSoftmax(nn.Module):
    ...
    def forward(self, attn_weights, attn_mask=None):
        ...
        if attn_mask is not None:
            # 确保 attn_mask 的形状与 attn_weights 匹配
            # attn_weights: [batch, heads, q_len, kv_len]
            # attn_mask: [batch, 1, q_len, kv_len] 或可能长度不匹配
            if attn_mask.shape[-1] != attn_weights.shape[-1]:
                # 切片 attn_mask 以匹配 attn_weights 的最后一个维度
                attn_mask = attn_mask[..., :attn_weights.shape[-1]]
            if attn_mask.shape[-2] != attn_weights.shape[-2]:
                # 切片 attn_mask 以匹配 attn_weights 的倒数第二个维度
                attn_mask = attn_mask[..., :attn_weights.shape[-2], :]
