...
def gptq_fwrd(model, tokenizer, dataloader, dev, args, bit_mask):
    ...
    for i in range(len(layers)):
        ...
        def save_gate_res_mixtral(module, inp, out):
            ...
            selected_experts.append(selected_expert.tolist())
        gate_handles = []
        if 'qwen' in args.model.lower() and hasattr(layer, "mlp") and hasattr(layer.mlp, "gate"):
            gate_handles.append(layer.mlp.gate.register_forward_hook(save_gate_res_qwen))
        elif 'deepseek' in args.model.lower() and hasattr(layer, "mlp") and hasattr(layer.mlp, "gate"):
            gate_handles.append(layer.mlp.gate.register_forward_hook(save_gate_res_deepseek))
        elif 'mixtral' in args.model.lower() and hasattr(layer, "block_sparse_moe") and hasattr(layer.block_sparse_moe, "gate"):
