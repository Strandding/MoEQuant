...
- Batched sigma computation for all parents at once
- Vectorized repetition penalty application
- Pre-allocated tensor buffers to reduce memory allocation overhead
- Optimized ngram checking with incremental hash
- Fused topk + filtering operations where possible
"""
...
class Candidate:
    ...
    ngram_hashes: Optional[Set[int]] = None  # cached ngram hashes for fast lookup
...
def _is_hf_cache(pkv: Any) -> bool:
    return (Cache is not None) and isinstance(pkv, Cache)
...
def _stack_past_key_values(pasts: List[Any]) -> Any:
    ...
    if _is_hf_cache(p0):
        ...
        if hasattr(p0, "key_cache") and hasattr(p0, "value_cache"):
            ...
            cache.key_cache = [torch.cat([p.key_cache[i] for p in pasts], dim=0) for i in range(len(p0))]
            cache.value_cache = [torch.cat([p.value_cache[i] for p in pasts], dim=0) for i in range(len(p0))]
            return cache
...
def _split_past_key_values(past_batched: Any, batch_size: int) -> List[Any]:
    ...
    if _is_hf_cache(past_batched):
        ...
        for bi in range(batch_size):
            ...
            if hasattr(pc, "batch_select_indices"):
...
# -----------------------------
# 4) Sigma computation (GPU) - optimized with pre-allocated buffers
# -----------------------------
@torch.no_grad()
def sigma_from_counts_gpu(counts: torch.Tensor, _buffer: Optional[Dict] = None) -> torch.Tensor:
    """
    counts: [B,L,E] int32 on GPU
    returns: [B] float32 sigma

    Optimized version with minimal tensor allocations.
    """
    assert counts.dim() == 3
    B, L, E = counts.shape
    if E <= 1:
    ...
    # Convert to float32 in-place-like manner
    counts_f = counts.to(dtype=torch.float32, non_blocking=True)

    # Compute frequencies: freqs = counts / sum(counts, dim=-1)
    T = counts_f.sum(dim=-1, keepdim=True).clamp_min_(1.0)  # [B,L,1], in-place clamp
    freqs = counts_f.div_(T)  # in-place division: [B,L,E]

    # Compute variance across experts
    mean = freqs.mean(dim=-1, keepdim=True)  # [B,L,1]
    freqs.sub_(mean)  # in-place: freqs = freqs - mean
    freqs.pow_(2)     # in-place: freqs = (freqs - mean)^2
    var = freqs.sum(dim=-1).div_(float(E - 1))  # [B,L], normalized variance

    # sigma = mean(sqrt(var)) across layers
    sig = var.sqrt_().mean(dim=-1)  # [B], in-place sqrt
    return sig


# Batch sigma computation for multiple independent count tensors
@torch.no_grad()
def sigma_from_counts_gpu_batch(counts_list: List[torch.Tensor]) -> torch.Tensor:
    """
    Compute sigma for a list of [L,E] count tensors.
    Returns [len(counts_list)] sigma values.
    More efficient than calling sigma_from_counts_gpu repeatedly.
    """
    if not counts_list:
        return torch.empty(0)

    # Stack into [B, L, E]
    counts_stacked = torch.stack(counts_list, dim=0)
    return sigma_from_counts_gpu(counts_stacked)
...
# -----------------------------
# 6) Update counts (GPU scatter_add) - optimized
# -----------------------------
@torch.no_grad()
def update_counts_from_gate_outputs_batch_gpu(
    ...
    moe_top_k: int,
    _ones_cache: Optional[Dict] = None,
):
    """
    ...
    Optimized with pre-allocated ones buffer.
    """
    ...
    # Pre-allocate ones tensor if needed (reuse across calls)
    if _ones_cache is not None and "ones" in _ones_cache:
        ones_template = _ones_cache["ones"]
        if ones_template.shape[0] < B or ones_template.shape[1] < moe_top_k:
            ones_template = torch.ones((B, moe_top_k), device=counts_b.device, dtype=counts_b.dtype)
            _ones_cache["ones"] = ones_template
    else:
        ones_template = None
    ...
    for li in range(use_L):
        ...
        topk_idx = torch.topk(x, k=k, dim=-1, sorted=False).indices  # [B,k], sorted=False is faster
        # If E2 != E, clamp indices into [0,E-1] (conservative)
        if E2 != E:
            topk_idx = topk_idx.clamp_(0, E - 1)  # in-place clamp

        if ones_template is not None:
            ones = ones_template[:B, :k]
        else:
            ones = torch.ones((B, k), device=counts_b.device, dtype=counts_b.dtype)
...
# Vectorized score computation for batch processing
@torch.no_grad()
def compute_score_or_cost_batch(
    rs_new: torch.Tensor,      # [N] float
    l_new: torch.Tensor,       # [N] int or float
    sigma_parent: torch.Tensor,  # [N] float
    tau: float,
) -> torch.Tensor:
    """Vectorized score computation on GPU."""
    l_new_f = l_new.float().clamp_min_(1.0)
    return -(rs_new / l_new_f) + (sigma_parent / tau)


# -----------------------------
# 9) Ngram helpers - optimized with rolling hash
# -----------------------------
_NGRAM_PRIME = 31
_NGRAM_MOD = 2**61 - 1

def _compute_ngram_hash(tokens: List[int], start: int, n: int) -> int:
    """Compute a rolling hash for ngram at position start."""
    h = 0
    for i in range(n):
        h = (h * _NGRAM_PRIME + tokens[start + i]) % _NGRAM_MOD
    return h


def _build_ngram_hashes(tokens: List[int], n: int) -> Set[int]:
    """Build set of ngram hashes from tokens."""
    if n <= 1 or len(tokens) < n:
        return set()
    hashes = set()
    for i in range(len(tokens) - n + 1):
        hashes.add(_compute_ngram_hash(tokens, i, n))
    return hashes


def _would_repeat_ngram_hash(tokens: List[int], v: int, n: int, ngram_hashes: Set[int]) -> bool:
    """Check if adding v would create a repeated ngram using cached hashes."""
    if n <= 1 or len(tokens) < n - 1:
        return False
    # Compute hash of potential new ngram (last n-1 tokens + v)
    h = 0
    for t in tokens[-(n - 1):]:
        h = (h * _NGRAM_PRIME + t) % _NGRAM_MOD
    h = (h * _NGRAM_PRIME + v) % _NGRAM_MOD
    return h in ngram_hashes


def _update_ngram_hashes(existing_hashes: Set[int], tokens: List[int], new_token: int, n: int) -> Set[int]:
    """Incrementally update ngram hashes after adding a new token."""
    if n <= 1:
        return existing_hashes
    new_hashes = existing_hashes.copy()
    if len(tokens) >= n - 1:
        # Add the new ngram formed by last n-1 tokens of (tokens) + new_token
        # But tokens doesn't include new_token yet, so we compute for tokens[-(n-1):] + [new_token]
        h = 0
        for t in tokens[-(n - 1):]:
            h = (h * _NGRAM_PRIME + t) % _NGRAM_MOD
        h = (h * _NGRAM_PRIME + new_token) % _NGRAM_MOD
        new_hashes.add(h)
    return new_hashes
...
# -----------------------------
# 10) EBSS batched generation: K sentences in parallel - optimized
# -----------------------------
def generate_sent_batch_ebss(
    ...
    _ones_cache = {"ones": torch.ones((K * width_w, moe_top_k), device=input_device, dtype=torch.int32)}
    ...
    update_counts_from_gate_outputs_batch_gpu(counts1, gate_outs_1, moe_top_k=moe_top_k, _ones_cache=_ones_cache)
    ...
    for i in range(K):
        initial_ngram_hashes = _build_ngram_hashes([first_tokens[i]], no_repeat_ngram_size) if no_repeat_ngram_size > 1 else set()
        beams.append([
            Candidate(
                ...
                ngram_hashes=initial_ngram_hashes,
            )
        ])

    # ---- Main loop (j from 1 to seq_len-1) ----
    for j in range(1, seq_len):
        # Collect all parent candidates across all sentences for batched sigma computation
        all_parents = []
        parent_indices = []  # (si, pi) for each parent
        for si in range(K):
            for pi, cand in enumerate(beams[si]):
                all_parents.append(cand)
                parent_indices.append((si, pi))

        if not all_parents:
            break

        # Batched sigma computation for all parents
        parent_counts_stacked = torch.stack([p.counts for p in all_parents], dim=0)  # [N, L, E]
        parent_sigmas = sigma_from_counts_gpu(parent_counts_stacked)  # [N]
        parent_sigma_map = {(si, pi): float(parent_sigmas[idx].item())
                           for idx, (si, pi) in enumerate(parent_indices)}
        ...
        for si in range(K):
            candidates = beams[si]
            expanded = []  # (val, pi, v, rs_new, ngram_hashes)

            for pi, cand in enumerate(candidates):
                # Use precomputed sigma
                sigma_parent = parent_sigma_map[(si, pi)]

                # Use cached ngram hashes
                ngram_hashes = cand.ngram_hashes if cand.ngram_hashes is not None else set()
                ...
                if repetition_penalty and repetition_penalty > 1.0:
                    used = list(set(cand.tokens))
                    if used:
                        idx = torch.tensor(used, device=logits.device, dtype=torch.long)
                        vals = logits[idx]
                        # Vectorized: where vals > 0, divide by penalty; else multiply
                        mask_pos = vals > 0
                        vals = torch.where(mask_pos, vals / repetition_penalty, vals * repetition_penalty)
                        logits[idx] = vals

                log_probs = torch.log_softmax(logits, dim=-1)
                k = min(top_token_k, log_probs.numel())
                topv = torch.topk(log_probs, k=k, dim=-1, sorted=False).indices.tolist()
                ...
                for v in topv:
                    ...
                    if max_repeat_run > 0 and _exceeds_repeat_run(cand.tokens, v, max_repeat_run):
                        continue
                    if no_repeat_ngram_size > 1 and _would_repeat_ngram_hash(cand.tokens, v, no_repeat_ngram_size, ngram_hashes):
                        continue
                    ...
                    # Pre-compute updated ngram hashes for this expansion
                    new_ngram_hashes = _update_ngram_hashes(ngram_hashes, cand.tokens, v, no_repeat_ngram_size)
                    expanded.append((val, pi, v, rs_new, new_ngram_hashes))
            ...
            for (val, pi, v, rs_new, new_ngram_hashes) in expanded:
        ...
        for item in selected_global:
            ...
            ngram_hashes_list.append(item["ngram_hashes"])
        ...
        update_counts_from_gate_outputs_batch_gpu(counts_b, gate_outs_b, moe_top_k=moe_top_k, _ones_cache=_ones_cache)
        ...
        for bi in range(B):
            ...
            cand_new = Candidate(
                ...
                ngram_hashes=ngram_hashes_list[bi],
            )
...
# -----------------------------
# 11) Build dataset (NS sentences) + incremental saving
# -----------------------------
def build_ebss_dataset(
    ...
    # Prefer single-device placement (avoid device_map="auto" overhead on single H100)
    ...
    try:
        ...
        model.to(device)
        print("[EBSS] attn_implementation=sdpa enabled.")
    except Exception as e:
        print(f"[EBSS][WARN] flash_attention_2 unavailable or failed ({type(e).__name__}: {e}). Falling back to default attention.")
        model = AutoModelForCausalLM.from_pretrained(
            model_path,
            torch_dtype=torch.bfloat16,
            trust_remote_code=trust_remote_code,
