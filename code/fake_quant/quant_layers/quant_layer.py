...
class QuantMoeBlock(nn.Module):
    def __init__(self, org_module: nn.Module, config: LlamaConfig, args):
        ...
        # Check if shared_expert exists (Qwen1.5/Qwen2 have it, Qwen3 doesn't)
    ...
    def forward(self, hidden_states: torch.Tensor,cur_sample: int=0) -> torch.Tensor:
        ...
        # Only use shared_expert if it exists (Qwen1.5/Qwen2 have it, Qwen3 doesn't)
...
class QuantAttention(nn.Module):
    def __init__(self, org_module: LlamaAttention, config: LlamaConfig, args):
        ...
        # Qwen3 explicitly specifies head_dim in config, don't calculate it
        # Qwen1.5/Qwen2 don't have explicit head_dim, calculate from hidden_size / num_heads
        ...
        # Handle different rotary_emb locations
        # Qwen1.5/Qwen2: rotary_emb is in attention layer
        # Qwen3: rotary_emb is at model level, need to create new instance
        if hasattr(org_module, 'rotary_emb'):
            self.rotary_emb = org_module.rotary_emb
        else:
            # For Qwen3 or other models where rotary_emb is not in attention layer
            # Create a new rotary embedding instance based on config
            # Check if we have a rotary_emb in args (passed from model level)
            if hasattr(args, 'rotary_emb'):
                self.rotary_emb = args.rotary_emb
            else:
                # Create a new instance based on model architecture
                # Try to import the appropriate RoPE class
                try:
                    # For Qwen3
                    ...
                    # Fallback: try to use LlamaRotaryEmbedding
    ...
    def forward(
        ...
        # 修复：正确计算 kv_seq_len
        # kv_seq_len 应该是当前序列长度加上 cache 中的长度
        kv_seq_len = q_len  # 当前输入的长度
...
class QuantDecoderLayer(nn.Module):
    ...
    def set_quant_state(
        ...
        # 1. 强制配置：保留权重量化意图，但强制关闭激活量化和全量化
        self.use_weight_quant = use_weight_quant
        self.use_act_quant = False    # 强制关闭激活量化
        self.use_fully_quant = False  # 强制关闭全量化

        # -------------------------------------------
        # A. 权重量化设置 (Weight Quantization)
        # -------------------------------------------

        # [Attention 部分]：强制关闭所有权重量化
        self.self_attn.q_proj.use_weight_quant = False
        self.self_attn.k_proj.use_weight_quant = False
        self.self_attn.v_proj.use_weight_quant = False
        ...
        # Norms
        self.input_layernorm.use_act_quant = False
        self.post_attention_layernorm.use_act_quant = False
        
        # Attention 内部激活
        self.self_attn.ropek.use_act_quant = False
        self.self_attn.v_proj.use_act_quant = False
        self.self_attn.pv_matmul.use_act_quant = False
        self.self_attn.q_proj.use_act_quant = False
        self.self_attn.k_proj.use_act_quant = False
        self.self_attn.ropeq.use_act_quant = False
        self.self_attn.qk_matmul.use_act_quant = False
        self.self_attn.softmax.use_act_quant = False
        ...
        self.resadd1.use_act_quant = False
        self.resadd2.use_act_quant = False

        for i in range(len(self.mlp.experts)):
            self.mlp.experts[i].mul.use_act_quant = False
            self.mlp.experts[i].silu.use_act_quant = False
            self.mlp.experts[i].gate_proj.use_act_quant = False
            self.mlp.experts[i].up_proj.use_act_quant = False
            self.mlp.experts[i].down_proj.use_act_quant = False
